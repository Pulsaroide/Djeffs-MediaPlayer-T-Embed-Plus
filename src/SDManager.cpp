...
bool init() {
    // SPI dédié SD — bus partagé avec display, mais CS séparés
    // DEDICATED_SPI évite les conflits de bus sur ESP32-S3
    SdSpiConfig spiConfig(SD_CS, DEDICATED_SPI, SD_SCK_MHZ(25));
    ...
    Serial.println("[SD] Init FAILED — vérifier SanDisk ≤32GB");
    return false;
}
...
// FIX: File → FsFile, FILE_READ → O_RDONLY  (SdFat v2 API)
FsFile openFile(const String& path) {
    return SD.open(path.c_str(), O_RDONLY);
}
